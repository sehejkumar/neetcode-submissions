class Solution:
    def mostVisitedPattern(self, username: List[str], timestamp: List[int], website: List[str]) -> List[str]:
        ...
        for user,website in patternMap.items():
            #generate combinations that are unique so we do not overcount users
            for currComb in set(combinations(website,3)):
                count[currComb] +=1
        #comparisons to get max pattern and max count
        maxPattern, maxCount = "",0
        for currComb, currCount in count.items():
            if currCount > maxCount or (currCount == maxCount) and currComb < maxPattern: #exicographically
                maxPattern = currComb
                maxCount = currCount
